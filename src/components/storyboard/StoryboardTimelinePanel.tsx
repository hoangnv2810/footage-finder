...
import type { StoryboardTimeline, StoryboardTimelineClip } from '@/lib/footage-app';
...
export function StoryboardTimelinePanel({
  ...
  const [removeClipTarget, setRemoveClipTarget] = useState<StoryboardTimelineClip | null>(null);
  ...
  const confirmRemoveClip = () => {
    if (!removeClipTarget || isBusy) return;
    onRemoveClip(removeClipTarget.id);
    setRemoveClipTarget(null);
  };
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-md bg-card/50">
                ...
                <li key={clip.id} data-testid={`timeline-clip-item-${clip.id}`} className="rounded-md border border-border/60 bg-background/25 px-2.5 py-2 transition-colors">
                  <div data-testid={`timeline-clip-title-row-${clip.id}`} className="flex items-center justify-between gap-2.5">
                    ...
                    <div className="flex shrink-0 items-center justify-end gap-0.5">
                      ...
                      <button
                        type="button"
                        onClick={() => setRemoveClipTarget(clip)}
                        disabled={isBusy}
      ...
      <Dialog open={Boolean(removeClipTarget)} onOpenChange={(open) => !open && setRemoveClipTarget(null)}>
        <DialogContent className="overflow-hidden rounded-md border-border bg-card p-0 sm:max-w-lg" aria-describedby={undefined}>
          <DialogHeader data-slot="timeline-dialog-header" className="border-b border-border px-4 py-2">
            <DialogTitle className="text-base font-semibold">Xóa clip khỏi timeline</DialogTitle>
          </DialogHeader>
          <div data-slot="timeline-dialog-body" className="space-y-2 px-4 py-2 text-sm">
            <p className="text-foreground">
              Bạn muốn xóa clip <span className="font-semibold">{removeClipTarget?.label}</span> khỏi timeline?
            </p>
            <div className="flex items-start gap-2 rounded-md border border-yellow-500/20 bg-yellow-500/10 px-3 py-2 text-xs text-yellow-600 dark:text-yellow-500">
              <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
              <p>Clip này chỉ bị xóa khỏi bản dựng, không xóa video gốc.</p>
            </div>
          </div>
          <DialogFooter data-slot="timeline-dialog-footer" className="px-4 pb-2 pt-1">
            <button type="button" onClick={() => setRemoveClipTarget(null)} className="rounded-md bg-secondary px-3 py-1.5 text-sm font-medium text-secondary-foreground transition-colors hover:bg-surface-hover">
              Hủy
            </button>
            <button type="button" onClick={confirmRemoveClip} disabled={isBusy} className="rounded-md bg-destructive px-3 py-1.5 text-sm font-medium text-destructive-foreground transition-colors hover:bg-destructive/90 disabled:cursor-not-allowed disabled:opacity-50">
              Xóa
            </button>
          </DialogFooter>
        </DialogContent>
