...
- dữ liệu cần được group theo `tên sản phẩm` để dễ quản lý
- `Thư viện dữ liệu` phải xem và thao tác video gần như đầy đủ, không chỉ là danh sách scenes tĩnh
...
- group thư viện theo `Product > video`
- thêm metadata `tên sản phẩm` ở mức batch mặc định và mức override theo video
- cho phép xem video, play scene, cắt tải xuống, export SRT, đổi version, xem matched/full ngay trong `Thư viện dữ liệu`
...
- group dữ liệu theo `Product > video`
- phân biệt nguồn `Extension` và `Web`
- lọc theo nguồn
- cho phép mở chi tiết từng dataset để xem version và scenes
- cho phép thao tác với video gần như đầy đủ ngay trong thư viện
- là nơi quay về để thấy trạng thái dữ liệu thật đã lưu trong DB

Danh sách thư viện hiển thị theo 2 cấp:

- cấp 1: nhóm sản phẩm
- cấp 2: các video/dataset thuộc sản phẩm đó

Mỗi group sản phẩm hiển thị tối thiểu:

- `productName`
- số video/dataset bên trong
...
- layout desktop dạng song song:
  - cột trái: metadata chính, version selector, video player
  - cột phải: toggle matched/full, scene list, action trên scene
- danh sách version
- version đang chọn
- scenes của version đó
- kết quả search đã lưu nếu có
- action gần như đầy đủ như màn `Tìm phân cảnh`:
  - play scene
  - cắt và tải xuống
  - export SRT
  - chuyển matched/full
  - xem các lượt search đã lưu
- metadata edit:
  - sửa `tên sản phẩm` cho video hiện tại
  - tùy chọn cập nhật `tên sản phẩm mặc định` cho batch nếu cần
...
`resolvedProductName` được tính theo rule:

1. dùng `history_video.product_name_override` nếu có
2. nếu không có thì dùng `history.product_name`
3. nếu vẫn không có thì fallback `Chưa gán sản phẩm`
...
Ngoài ra cần thêm metadata sản phẩm:

- `history.product_name`
- `history_video.product_name_override`
...
Payload cũng cần bổ sung metadata sản phẩm:

- `history.productName`
- `video.productNameOverride`
- có thể thêm luôn `video.resolvedProductName` nếu muốn giảm logic ở frontend

Ngoài ra cần API cập nhật metadata:

- cập nhật `product_name` ở mức history mặc định
- cập nhật `product_name_override` ở mức dataset/video
...
3. Nếu extension có `product_name`, backend lưu vào metadata phù hợp
4. Dataset tương ứng được đánh dấu `source=extension`
5. `Thư viện dữ liệu` hiển thị dataset đó trong đúng group sản phẩm với nhãn `Extension`
6. User có thể mở video, xem player/scenes, cắt clip, export, và dùng ngay cho storyboard
...
Ở màn thư viện còn có context nhóm:
...
Grouping theo sản phẩm chỉ là lớp tổ chức ở UI và metadata. Nó không được làm thay đổi engine storyboard hay contract `selected_version_ids`.
...
Policy metadata cho duplicate import:

- nếu payload import mới có `product_name` nhưng DB đang trống thì fill vào
- nếu DB đã có metadata sản phẩm thì không overwrite tự động
- chỉnh tay trong web UI là nguồn quyết định cuối
...
Nếu xóa dataset là video cuối cùng trong một group sản phẩm, group đó cũng phải biến mất khỏi thư viện.

Nếu đổi `productName` của một dataset, dataset phải chuyển sang đúng group sản phẩm mới ngay sau khi save.
