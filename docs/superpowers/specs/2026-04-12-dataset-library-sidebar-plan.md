...
- `Thư viện dữ liệu` là nơi quản lý chính: cần xem video và thao tác clip gần như đầy đủ ngay tại đây.
- Group thư viện theo `Product > video`.
- `tên sản phẩm` là metadata chính thức và hỗ trợ cả batch-default lẫn override theo video.
...
## Phase 1B: Bổ sung metadata sản phẩm ở backend

### Mục tiêu

Persist được `tên sản phẩm` ở cả mức batch mặc định và mức override theo video.
...
1. Thêm `product_name` vào bảng `history`.
2. Thêm `product_name_override` vào bảng `history_video`.
3. Nếu cột chưa tồn tại, chạy migration với giá trị mặc định rỗng.
4. Cập nhật payload `GET /api/history` để trả:
   - `history.productName`
   - `video.productNameOverride`
   - tùy chọn `resolvedProductName`
5. Cập nhật các luồng ghi dữ liệu để nhận metadata sản phẩm nếu có:
   - import từ extension
   - save history tổng quát
6. Duplicate import policy:
   - nếu payload import mới có `product_name` và DB đang trống thì fill vào
   - nếu DB đã có metadata thì không overwrite tự động
7. Thêm API update metadata:
   - update `product_name` ở mức history
   - update `product_name_override` ở mức dataset/video

### Kết quả mong đợi

- backend trả đủ metadata để frontend group theo sản phẩm
- user có thể sửa `tên sản phẩm` từ web UI
...
7. Thêm selector group theo `resolvedProductName`.
...
- dataset được group theo sản phẩm để dễ quản lý
...
6. Detail của dataset hiển thị:
    ...
    - video player
    - trim/export/play scene
    - desktop layout side-by-side giống màn `Tìm phân cảnh`, không đẩy scene list xuống dưới player
7. Đổi wording nút xóa để phản ánh xóa dữ liệu đã lưu, không phải xóa lịch sử tìm kiếm.
8. Group list theo `Product > video`.
9. Hiển thị và chỉnh `tên sản phẩm` ngay trong detail panel.

### Kết quả mong đợi

- `Thư viện dữ liệu` là nơi xem dữ liệu đã lưu trong DB, bất kể nguồn nào
- `Thư viện dữ liệu` là nơi quản lý chính theo `Product > video`
...
6. Tách khối video detail dùng chung để không copy-paste logic giữa `Tìm phân cảnh` và `Thư viện dữ liệu`.
...
7. Nếu dataset active chưa usable, không tự chọn dataset khác; chỉ hiển thị trạng thái rõ ràng.
...
6. Nếu đổi `productName` hoặc xóa dataset, group và selection phải đồng bộ lại ngay.
...
3. Thêm empty state riêng cho:
    ...
    - product group không có video nào
...
6. Nếu có nhiều lượt search đã lưu, thư viện phải cho xem và áp lại từng lượt search đó.
...
6. Chọn dataset xem được version/scenes/video player ngay trong thư viện.
...
10. Group theo sản phẩm hoạt động đúng.
11. Sửa `tên sản phẩm` làm dataset chuyển group đúng.
12. Trim/export từ thư viện hoạt động như ở màn tìm phân cảnh.
...
2. Backend metadata sản phẩm và API update metadata
3. Frontend flatten `HistoryItem[] -> DatasetItem[]`
4. Group selector `Product > video`
5. Sidebar navigation shell
6. Refactor shared `VideoDetailPanel`
7. `Thư viện dữ liệu`
8. `Tìm phân cảnh`
9. `Storyboard`
10. Copy, empty state, error state
